...
public class WayEditor : Editor
{
    ...
    void OnSceneGUI()
    {
        Way polyline = (target as Way);
        Vector3[] localPoints = polyline.Waypoints.ToArray();
        Vector3[] worldPoints = new Vector3[polyline.Waypoints.Count];
        ...
        if (Event.current.shift)
        {
            ...
            if (Handles.Button(mousePos, Quaternion.identity, handleSize * 0.1f, handleSize, HandleFunc))
            {
                ...
                polyline.Waypoints.Insert(nodeIndex + 1, polyLocalMousePos);
                Event.current.Use();
            }
        }
        if (Event.current.control)
        {
            ...
            if (Handles.Button(worldPoints[indexToDelete], Quaternion.identity, handleSize * 0.09f, handleSize, DeleteHandleFunc))
            {
                ...
                polyline.Waypoints.RemoveAt(indexToDelete);
                indexToDelete = -1;
                Event.current.Use();
            }
        ...
        for(int i=0;i<nodes.Length-1;i++)
        {
            if(alignedPoints.Contains(i)&&alignedPoints.Contains(i+1))
            {
                ...
                if (i + 1 == nodes.Length - 1)
                {
                    ...
                    Handles.DrawDottedLine(nodes[i + 1], nodes[0], 10);
                    Handles.color = Color.green;
                }
                Handles.color = currentColor;
            }
    ...
    private void DrawNodes(Way polyline, Vector3[] worldPoints)
    {
        for (int i = 0; i < polyline.Waypoints.Count; i++)
        {
            Vector3 pos = polyline.transform.TransformPoint(polyline.Waypoints[i]);
            float handleSize = HandleUtility.GetHandleSize(pos);
            Vector3 newPos = Handles.FreeMoveHandle(pos, Quaternion.identity,
                handleSize * 0.09f, Vector3.one, HandleFunc);
            (target as Way).OnValidate();
            if (newPos != pos)
            {
                CheckAlignment(worldPoints, handleSize * 0.1f, i, ref newPos);
                Undo.RecordObject(polyline, "Move Node");
                polyline.Waypoints[i] = polyline.transform.InverseTransformPoint(newPos);
            }
        }
    }
